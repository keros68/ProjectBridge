...
- **网页修改**：默认开启 YOLO，ChatGPT 的修改直接写入，写入前自动备份，可恢复。可按项目关闭，改为先生成预览、本机确认后写入。
...
程序启动时会检查新版本，有更新会在窗口顶部提示。点“立即更新”即可：程序下载并校验安装程序，自动退出、安装到当前目录并重新启动，期间连接会断开一次。项目和连接设置保存在用户目录，升级和卸载都不受影响。可在“设置 → 更新”中关闭检查。
...
| | 快速体验 | 长期使用 |
| --- | --- | --- |
| 技术方式 | OAuth 临时连接 | OpenAI Secure Tunnel |
| 额外配置 | 无 | 需要 Tunnel ID 和具有 Tunnels Read + Use 权限的运行密钥 |
| 公网地址 | 程序重启后可能变化，变化后需在 ChatGPT 中更新 | 固定，插件长期复用 |
| 适合 | 首次使用 | 日常使用 |

不确定时选“快速体验”，之后可随时改用固定入口。Secure Tunnel 所需的 Tunnel ID 和运行密钥由向导中的官方入口创建，粘贴后保存到 Windows 凭据管理器。

**4. 在 ChatGPT 中添加插件**

通道就绪后，首次设置窗口会直接展开 ChatGPT 引导，按三步操作：
...
连接建立后回到首页。新人任务卡会提示添加一个本地项目，默认只授权网页读取；建立连接本身不会自动暴露其他目录。

**6. 验证项目读取**

点击新人任务卡中的“复制项目验证提示词”，在 ChatGPT 中选中 ProjectBridge 后粘贴发送。提示词会先调用 `list_projects`，再对该项目调用 `list_directory` 列出根目录，不读取文件内容。成功后新人任务卡自动完成并隐藏。
...
- **托盘图标是蓝色，但显示未验证**：蓝色表示通道已连接，验证需要网页实际调用一次工具，见第 6 步。
...
- YOLO 默认开启，网页连接验证后对已授权网页读取的项目生效，断开或退出时收回；在“编辑”页取消勾选可为单个项目改为本机确认，并记住该选择。Codex 写入权限需要每次连接后手动开启。
